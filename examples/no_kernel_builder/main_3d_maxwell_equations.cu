...
// Perfect conductor boundary condition (tangential E = 0)
Eigen::Matrix<float, -1, 1> perfect_conductor_bc(Mesh3D *mesh, Eigen::Matrix<float, -1, 1> &arr, const DT *dt_) {
    return arr.cwiseProduct(mesh->_node_is_boundary_reverse);
}

// CUDA version - perfect conductor boundary condition (simple, so use native CUDA for performance)
CudaDataMatrixD perfect_conductor_bc_cu(Mesh3D *mesh, CudaDataMatrixD &arr, const DT *dt_) {
...
auto create_create_dipole_source_bc_cu(const DipoleSource &dipole) {
    return [&dipole](Mesh3D *mesh, CudaDataMatrixD &arr, const DT *dt_) {
        ...
        CudaDataMatrixD arr_n = arr * cu_mesh->_node_is_boundary_reverse_cu;
...
int main(int argc, char **argv) {
    ...
    auto Ex = Variable(mesh.get(), zero_field, perfect_conductor_bc, perfect_conductor_bc_cu, "Ex");
    auto Ey = Variable(mesh.get(), zero_field, perfect_conductor_bc, perfect_conductor_bc_cu, "Ey");
    auto Ez = Variable(mesh.get(), zero_field, dipole_bc, dipole_bc_cu, "Ez");
    auto Hx = Variable(mesh.get(), zero_field, perfect_conductor_bc, perfect_conductor_bc_cu, "Hx");
    auto Hy = Variable(mesh.get(), zero_field, perfect_conductor_bc, perfect_conductor_bc_cu, "Hy");
    auto Hz = Variable(mesh.get(), zero_field, perfect_conductor_bc, perfect_conductor_bc_cu, "Hz");
    ...
    EquationTemplate equation_system = {
        ...
        {&Ex, '=', Ex + dt * (d1dy(Hz) - d1dz(Hy)), false},
        {&Ey, '=', Ey + dt * (d1dz(Hx) - d1dx(Hz)), false},
        {&Ez, '=', Ez + dt * (d1dx(Hy) - d1dy(Hx)), false},

        // Update magnetic field: H^{n+1} = H^n - dt * (∇ × E^{n+1})
        {&Hx, '=', Hx - dt * (d1dy(Ez) - d1dz(Ey)), false},
        {&Hy, '=', Hy - dt * (d1dz(Ex) - d1dx(Ez)), false},
        {&Hz, '=', Hz - dt * (d1dx(Ey) - d1dy(Ex)), false},
    };
