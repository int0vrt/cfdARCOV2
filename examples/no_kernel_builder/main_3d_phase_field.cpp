...
auto create_boundary() {
    return [](Mesh3D *mesh, Eigen::Matrix<float, -1, 1> &arr, const DT *dt_) {
        return arr.cwiseProduct(mesh->_node_is_boundary_reverse);
    };
}

auto create_boundary_cu() {
    return [](Mesh3D *mesh, CudaDataMatrixD &arr, const DT *dt_) {
...
int main(int argc, char** argv) {
    ...
    auto phi = Variable(mesh.get(), phi_initial, create_boundary(), create_boundary_cu(), "phi");
    auto T = Variable(mesh.get(), T_initial, create_boundary(), create_boundary_cu(), "T");

    auto dt = DT(mesh.get(), UpdatePolicies::constant_dt, UpdatePolicies::constant_dt_cu, initializer.dt, {&phi, &T});

    EquationTemplate eqs = {
            {&phi, '=', phi + dt * (W * W * lapl(phi) - (phi * (phi * phi - 1.0f) + lambda * (T - T_M) * (1.0f - phi * phi))) / tau, false},
            {&T,   '=', T + dt * (D * lapl(T) + (L / 2.0f) * phi * dt), false}
    };
