...
// CPU version - periodic boundary condition (zero gradient)
Eigen::Matrix<float, -1, 1> periodic_bc(Mesh3D *mesh, Eigen::Matrix<float, -1, 1> &arr, const DT *dt_) {
    return arr.cwiseProduct(mesh->_node_is_boundary_reverse);
}

// CUDA version - periodic boundary condition (zero gradient)
CudaDataMatrixD periodic_bc_cu(Mesh3D *mesh, CudaDataMatrixD &arr, const DT *dt_) {
...
int main(int argc, char **argv) {
    ...
    auto u = Variable(mesh.get(), init_conditions.u_init, periodic_bc, periodic_bc_cu, "u");
    auto v = Variable(mesh.get(), init_conditions.v_init, periodic_bc, periodic_bc_cu, "v");
    ...
    EquationTemplate equation_system = {
        ...
        {&u, '=', u + dt * (Du * (d2dx(u) + d2dy(u)) + A - (B + 1.0f) * u + u * u * v), false},
            // v-component: ∂v/∂t = Dv∇²v + Bu - u²v
        {&v, '=', v + dt * (Dv * (d2dx(v) + d2dy(v)) + B * u - u * u * v), false}
    };
