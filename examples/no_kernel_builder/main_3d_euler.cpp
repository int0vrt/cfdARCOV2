...
_boundary_copy(Mesh3D *mesh, Eigen::Matrix<float, -1, 1> &arr, const Eigen::Matrix<float, -1, 1> &copy_var) {
...
auto boundary_copy(const Eigen::Matrix<float, -1, 1> &copy_var) {
    return [copy_var](Mesh3D *mesh, Eigen::Matrix<float, -1, 1> &arr, const DT *dt_) {
        return _boundary_copy(mesh, arr, copy_var);
    };
}

CudaDataMatrixD
_boundary_copy_cu(Mesh3D *mesh_in, CudaDataMatrixD &arr, const CudaDataMatrixD &copy_var, const DT *dt_) {
...
int main(int argc, char **argv) {
    ...
    auto rho_u = Variable(mesh.get(), rho_u_initial, boundary_copy(rho_u_initial), boundary_copy_cu(rho_u_initial),
                          "rho_u");

    Eigen::Matrix<float, -1, 1> rho_v_initial = rho.current.array() * v.current.array() * mesh->_volumes.array();
    auto rho_v = Variable(mesh.get(), rho_v_initial, boundary_copy(rho_v_initial), boundary_copy_cu(rho_v_initial),
                          "rho_v");

    Eigen::Matrix<float, -1, 1> rho_w_initial = rho.current.array() * w.current.array() * mesh->_volumes.array();
    auto rho_w = Variable(mesh.get(), rho_w_initial, boundary_copy(rho_w_initial), boundary_copy_cu(rho_w_initial),
                          "rho_w");
    ...
    EquationTemplate equation_system = {
            {&rho, '=', mass / volumes_var,                                                        false},
            {&u,   '=', rho_u / rho / volumes_var,                                                 false},
            {&v,   '=', rho_v / rho / volumes_var,                                                 false},
            {&w,   '=', rho_w / rho / volumes_var,                                                 false},
            {&p,   '=', (rho_e / volumes_var - 0.5 * rho * (u * u + v * v + w * w)) * (gamma - 1), false},

            {&rho, '=', rho - 0.5 * dt * (u * d1dx(rho) + rho * d1dx(u) + v * d1dy(rho) + rho * d1dy(v) + w * d1dz(rho) + rho * d1dz(w)), false},
            {&u,   '=', u - 0.5 * dt * (u * d1dx(u) + v * d1dy(u) + w * d1dz(u) + (1 / rho) * d1dx(p)),                    false},
            {&v,   '=', v - 0.5 * dt * (u * d1dx(v) + v * d1dy(v) + w * d1dz(v) + (1 / rho) * d1dy(p)),                    false},
            {&w,   '=', w - 0.5 * dt * (u * d1dx(w) + v * d1dy(w) + w * d1dz(w) + (1 / rho) * d1dz(p)),                    false},
            {&p,   '=', p - 0.5 * dt * (gamma * p * (d1dx(u) + d1dy(v) + d1dz(w)) + u * d1dx(p) + v * d1dy(p) + w * d1dz(p)), false},

            {&mass_tmp,  '=', -(d1dx(rho * u) + d1dy(rho * v) + d1dz(rho * w)),                 false},
            {&rho_u_tmp, '=', -(d1dx(rho * u * u + p) + d1dy(rho * v * u) + d1dz(rho * w * u)), false},
            {&rho_v_tmp, '=', -(d1dx(rho * v * u) + d1dy(rho * v * v + p) + d1dz(rho * v * w)), false},
            {&rho_w_tmp, '=', -(d1dx(rho * w * u) + d1dy(rho * w * v) + d1dz(rho * w * w + p)), false},
            {&rho_e_tmp, '=', -(d1dx((E + p) * u) + d1dy((E + p) * v) + d1dz((E + p) * w)),     false},
