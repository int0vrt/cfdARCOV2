...
// CPU version - Dirichlet boundary condition with pre-computed source
auto create_heat_boundary_condition(const Eigen::Matrix<float, -1, 1> &boundary_source) {
    return [&boundary_source](Mesh3D *mesh, Eigen::Matrix<float, -1, 1> &arr, const DT *dt_) {
        return arr.cwiseProduct(mesh->_node_is_boundary_reverse) + boundary_source;
    };
}

// CUDA version - Dirichlet boundary condition with pre-computed source
auto create_heat_boundary_condition_cu(const CudaDataMatrixD &boundary_source) {
    return [&boundary_source](Mesh3D *mesh, CudaDataMatrixD &arr, const DT *dt_) {
        auto *cuda_mesh = dynamic_cast<CudaMesh3D *>(mesh);
        return arr * cuda_mesh->_node_is_boundary_reverse_cu + boundary_source;
    };
}
...
int main(int argc, char **argv) {
    ...
    auto boundary_source = compute_heat_boundary_source(mesh.get());
    auto boundary_source_cu = CudaDataMatrixD::from_eigen(boundary_source);

    // Create boundary condition factories
    auto heat_bc = create_heat_boundary_condition(boundary_source);
    auto heat_bc_cu = create_heat_boundary_condition_cu(boundary_source_cu);

    Eigen::Matrix<float, -1, 1> T_initial = initial_T(mesh.get());
    auto T = Variable(mesh.get(), T_initial, heat_bc, heat_bc_cu, "T");
