...
_boundary_copy_2d_via_3d(Mesh3D *mesh, Eigen::Matrix<float, -1, 1> &arr, const Eigen::Matrix<float, -1, 1> &copy_var) {
    ...
    for (auto &node: mesh->_nodes) {
        if (x_limit_lower < node.x() && node.x() < x_limit_upper && y_limit_lower < node.y() &&
            node.y() < y_limit_upper) {
            ret(i) = arr(i);
        } else {
            ret(i) = copy_var(i);
        }
...
auto boundary_zero(const Eigen::Matrix<float, -1, 1> &copy_var) {
    return [copy_var](Mesh3D *mesh, Eigen::Matrix<float, -1, 1> &arr, const DT *dt_) {
        return _boundary_zero_2d_via_3d(mesh, arr);
    };
}
...
//    auto u = Variable(mesh.get(), initial_, boundary_copy(_zero), boundary_copy_cu(_zero), "u");
    ...
    if (CFDArcoGlobalInit::get_rank() == 0) {
        std::cout << "\n2D Wave Equation (Gaussian Pulse) Simulation Completed!" << std::endl;
        std::cout << "Wave speed (c): " << c << std::endl;
        std::cout << "Initial condition: Sinusoidal wave pattern" << std::endl;
        std::cout << "Boundary condition: Zero boundary (Dirichlet)" << std::endl;
        std::cout << "Expected behavior: Wave propagation and reflections" << std::endl;
