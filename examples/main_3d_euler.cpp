...
Eigen::Matrix<float, -1, 1> initial_rho(Mesh3D *mesh) {
    ...
    for (auto &node: mesh->_nodes) {
        if ((y_limit_lower < node.y() && node.y() < y_limit_upper) &&
            (x_limit_lower < node.x() && node.x() < x_limit_upper) && node.z() < z_limit_upper) {
            ret(i) = 5;
        } else {
            ret(i) = 1;
        }
...
Eigen::Matrix<float, -1, 1> initial_w(Mesh3D *mesh) {
    ...
    for (auto &node: mesh->_nodes) {
        if ((y_limit_lower < node.y() && node.y() < y_limit_upper) &&
            (x_limit_lower < node.x() && node.x() < x_limit_upper)) {
            ret(i) = W_SPEED;
        } else {
            ret(i) = W_SPEED;
        }
...
int main(int argc, char **argv) {
    ...
    auto rho = Variable(mesh.get(), rho_initial, boundary_copy(rho_initial), boundary_copy_cu(mesh.get(), rho_initial), "rho");

    auto u_initial = initial_u(mesh.get());
    auto u = Variable(mesh.get(), u_initial, boundary_copy(u_initial), boundary_copy_cu(mesh.get(), u_initial), "u");

    auto v_initial = initial_v(mesh.get());
    auto v = Variable(mesh.get(), v_initial, boundary_copy(v_initial), boundary_copy_cu(mesh.get(), v_initial), "v");

    auto w_initial = initial_w(mesh.get());
    auto w = Variable(mesh.get(), w_initial, boundary_copy(w_initial), boundary_copy_cu(mesh.get(), w_initial), "w");

    Eigen::Matrix<float, -1, 1> p_initial = Eigen::Matrix<float, -1, 1>{mesh->_num_nodes};
    p_initial.setConstant(2.5);
    auto p = Variable(mesh.get(), p_initial, boundary_copy(p_initial), boundary_copy_cu(mesh.get(), p_initial), "p");

    Eigen::Matrix<float, -1, 1> mass_initial = rho.current.array() * mesh->_volumes.array();
    auto mass = Variable(mesh.get(), mass_initial, boundary_copy(mass_initial), boundary_copy_cu(mesh.get(), mass_initial), "mass");

    Eigen::Matrix<float, -1, 1> rho_u_initial = rho.current.array() * u.current.array() * mesh->_volumes.array();
    auto rho_u = Variable(mesh.get(), rho_u_initial, boundary_copy(rho_u_initial), boundary_copy_cu(mesh.get(), rho_u_initial),
                          "rho_u");

    Eigen::Matrix<float, -1, 1> rho_v_initial = rho.current.array() * v.current.array() * mesh->_volumes.array();
    auto rho_v = Variable(mesh.get(), rho_v_initial, boundary_copy(rho_v_initial), boundary_copy_cu(mesh.get(), rho_v_initial),
                          "rho_v");

    Eigen::Matrix<float, -1, 1> rho_w_initial = rho.current.array() * w.current.array() * mesh->_volumes.array();
    auto rho_w = Variable(mesh.get(), rho_w_initial, boundary_copy(rho_w_initial), boundary_copy_cu(mesh.get(), rho_w_initial),
                          "rho_w");
    ...
    Eigen::Matrix<float, -1, 1> E_initial = (p.current.array() / (gamma - 1) + 0.5 * rho.current.array() *
                                                                               (u.current.array() * u.current.array() +
                                                                                v.current.array() * v.current.array() +
                                                                                w.current.array() *
                                                                                w.current.array())) *
                                            mesh->_volumes.array();
    auto rho_e = Variable(mesh.get(), E_initial, boundary_copy(E_initial), boundary_copy_cu(mesh.get(), E_initial), "rho_e");
    ...
    auto dt = DT(mesh.get(), UpdatePolicies::CourantFriedrichsLewy3D, UpdatePolicies::CourantFriedrichsLewy3DCu,
    ...
    EquationTemplate equation_system = {
            NEntryVar{{&rho, '=', mass / volumes_var,                                                        true},
                      {&u,   '=', rho_u / rho / volumes_var,                                                 true},
                      {&v,   '=', rho_v / rho / volumes_var,                                                 true},
                      {&w,   '=', rho_w / rho / volumes_var,                                                 true},
                      {&p,   '=', (rho_e / volumes_var - 0.5 * rho * (u * u + v * v + w * w)) * (gamma -
                                                                                                 1),         true}}.restruct(),

            NEntryVar{{&rho, '=', rho - 0.5 * dt *
                                        (u * d1dx(rho) + rho * d1dx(u) + v * d1dy(rho) + rho * d1dy(v) + w * d1dz(rho) +
                                         rho * d1dz(w)),                                                                 true},
                      {&u,   '=', u - 0.5 * dt * (u * d1dx(u) + v * d1dy(u) + w * d1dz(u) + (1 / rho) *
                                                                                            d1dx(p)),                    true},
                      {&v,   '=', v - 0.5 * dt * (u * d1dx(v) + v * d1dy(v) + w * d1dz(v) + (1 / rho) *
                                                                                            d1dy(p)),                    true},
                      {&w,   '=', w - 0.5 * dt * (u * d1dx(w) + v * d1dy(w) + w * d1dz(w) + (1 / rho) *
                                                                                            d1dz(p)),                    true},
                      {&p,   '=', p - 0.5 * dt *
                                      (gamma * p * (d1dx(u) + d1dy(v) + d1dz(w)) + u * d1dx(p) + v * d1dy(p) + w *
                                                                                                               d1dz(p)), true}}.restruct(),

            NEntryVar{{&mass_tmp,  '=', -(d1dx(rho * u) + d1dy(rho * v) + d1dz(rho * w)),                 true},
                      {&rho_u_tmp, '=', -(d1dx(rho * u * u + p) + d1dy(rho * v * u) + d1dz(rho * w * u)), true},
                      {&rho_v_tmp, '=', -(d1dx(rho * v * u) + d1dy(rho * v * v + p) + d1dz(rho * v * w)), true},
                      {&rho_w_tmp, '=', -(d1dx(rho * w * u) + d1dy(rho * w * v) + d1dz(rho * w * w + p)), true},
                      {&rho_e_tmp, '=', -(d1dx((E + p) * u) + d1dy((E + p) * v) + d1dz((E + p) *
                                                                                       w)),               true}}.restruct(),
    ...
    if (CFDArcoGlobalInit::get_rank() == 0) {
        std::cout << "\n3D Euler Equations Simulation Completed!" << std::endl;
        std::cout << "Specific heat ratio (γ): " << gamma << std::endl;
        std::cout << "Initial density ratio: 5:1 (high density region in center)" << std::endl;
        std::cout << "Velocity pattern: Alternating layers with constant w-velocity" << std::endl;
        std::cout << "Artificial dissipation: " << dissip << std::endl;
        std::cout << "Boundary condition: Copy boundary (inflow/outflow)" << std::endl;
        std::cout << "Expected features: Compressible flow, density waves, shock formation" << std::endl;
