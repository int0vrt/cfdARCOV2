...
//}
//
//void CFDArcoGlobalInit::make_node_distribution(Mesh3D *_mesh, DistributionStrategy distribution_strategy,
...
////    MatrixX4dRB buff {mesh->_num_nodes, inst.cols()};
////    buff.setConstant(0);
////    buff(current_proc_node_distribution, Eigen::all) = inst;
////
////    std::vector<MatrixX4dRB> input_buffers;
...
//rmm::mr::managed_memory_resource *get_cuda_resource() {
//    static rmm::mr::managed_memory_resource res{};
//    return &res;
//}
...
void CFDArcoGlobalInit::enable_cuda(int cuda_ranks, bool use_cuda, bool use_hip) {

#if !defined(CFDARCHO_CUDA_ENABLE)
    if (use_cuda) throw std::runtime_error{"CUDA not available"};
#endif
#if !defined(CFDARCHO_HIP_ENABLE)
    if (use_hip) throw std::runtime_error{"HIP not available"};
#endif
    ...
    if (cuda_enabled) {
        ...
        Allocator::cuda_mem_pool = std::make_unique<rmm::mr::pool_memory_resource<rmm::mr::cuda_memory_resource>>(
                get_cuda_resource(),
                rmm::detail::align_up(static_cast<std::size_t>(static_cast<float>(free) * (.95 / cuda_ranks)),
                                      rmm::detail::CUDA_ALLOCATION_ALIGNMENT),
                rmm::detail::align_up(static_cast<std::size_t>(static_cast<float>(free) * (1.0 / cuda_ranks)),
                                      rmm::detail::CUDA_ALLOCATION_ALIGNMENT)
        );
